package me.WesBag.Toontown.BattleCore.Cogs.Sellbot.NameDropper;

public class NameDropper {

	public static final int[][] RazzleDazzle = {
			{30, 30, 30, 30, 30},
			{4, 5, 6, 9, 12},
			{75, 80, 85, 90, 95}
	};
	public static final int[][] Rolodex = {
			{40, 40, 40, 40, 40},
			{5, 6, 7, 10, 14},
			{95, 95, 95, 95, 95}
	};
	public static final int[][] Synergy = {
			{15, 15, 15, 15, 15},
			{3, 4, 6, 9, 12},
			{50, 50, 50, 50, 50}
	};
	public static final int[][] PickPocket = {
			{15, 15, 15, 15, 15},
			{2, 2, 2, 2, 2},
			{95, 95, 95, 95, 95}
	};
	public static final int numOfAttacks = 4;
	public static final int[][][] attacks = {RazzleDazzle, Rolodex, Synergy, PickPocket};
	public static final String[] attackNames = {"Razzle Dazzle", "Rolodex", "Synergy", "Pick Pocket"};
	public static final boolean[] hitAll = {false, false, true, false};
	public static final String skinTexture = "";
	public static final String skinSignature = "";
}
